 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "# Grabbing Data From Yahoo Finance For Stock Build last updated 13/12/2025"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "    \n",
    "    df = df.copy()\n",
    "    print(\"[seed-path] holdings: columns ->\", list(df.columns))\n",
    "    df.columns = [str(c).strip() for c in df.columns]\n",
    "    df[\"Security\"] = df[\"Security\"].astype(str).str.strip()\n",
    "    # Accept common variants of the Units header\n",
    "    if \"Units\" not in df.columns:\n",
    "        for alt in [\"Curr Units\", \"Current Units\", \"Holdings\", \"Qty\"]:\n",
    "            if alt in df.columns: df[\"Units\"] = df[alt]; break\n",
    "    df.columns = [str(c).strip() for c in df.columns]\n",
    "    df[\"Security\"] = df[\"Security\"].astype(str).str.strip()\n",
    "\n",
  ...
  },
  {
   ...
   "outputs": [
    ...
    {
     "ename": "NameError",
     "evalue": "name 'mu_plus' is not defined",
     "output_type": "error",
     "traceback": [
      "\u001b[1;31m---------------------------------------------------------------------------\u001b[0m",
      "\u001b[1;31mNameError\u001b[0m                                 Traceback (most recent call last)",
      "Cell \u001b[1;32mIn[24], line 660\u001b[0m\n\u001b[0;32m    655\u001b[0m     \u001b[38;5;28;01mreturn\u001b[39;00m W, stats_df, tan_ret, tan_vol\n\u001b[0;32m    657\u001b[0m \u001b[38;5;66;03m# ------------------------------------------------------------\u001b[39;00m\n\u001b[0;32m    658\u001b[0m \u001b[38;5;66;03m# 9) FRONTIERS: MVP-centred long-only frontier\u001b[39;00m\n\u001b[0;32m    659\u001b[0m \u001b[38;5;66;03m# ------------------------------------------------------------\u001b[39;00m\n\u001b[1;32m--> 660\u001b[0m mu_frontier \u001b[38;5;241m=\u001b[39m mu_plus\u001b[38;5;241m.\u001b[39mcopy()\n\u001b[0;32m    661\u001b[0m Sigma_frontier \u001b[38;5;241m=\u001b[39m cov_plus\u001b[38;5;241m.\u001b[39mcopy()\n\u001b[0;32m    663\u001b[0m W, stats_df, tan_ret, tan_vol \u001b[38;5;241m=\u001b[39m _build_frontier(\n\u001b[0;32m    664\u001b[0m     mu_frontier,\n\u001b[0;32m    665\u001b[0m     Sigma_frontier,\n\u001b[1;32m   (...)\u001b[0m\n\u001b[0;32m    668\u001b[0m     max_excess_return\u001b[38;5;241m=\u001b[39m\u001b[38;5;241m0.95\u001b[39m,\n\u001b[0;32m    669\u001b[0m )\n",
      "\u001b[1;31mNameError\u001b[0m: name 'mu_plus' is not defined"
     ]
    }
   ],
   "source": [
    ...
    "# ------------------------------------------------------------\n",
    "mu_frontier = mu_plus.copy()\n",
    "Sigma_frontier = cov_plus.copy()\n",
    "\n",
    ...
    "\n",
    "    # 5) Small-trade suppressor (±2 units → no trade)\n",
    "    delta = (tgt_units - cur_units).astype(int)\n",
    "    small_mask = delta.abs() <= 2\n",
    "    tgt_units.loc[small_mask] = cur_units.loc[small_mask]\n",
    "    delta = (tgt_units - cur_units).astype(int)\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    sht.autofit()\n",
    "\n",
    "def update_efficient_frontier_chart(opt_sheet, stats_df, start_s_row, rf_annual, tan_ret, tan_vol, current_point, title_text):\n",
    ...
    "            ch = co.Item(i).Chart\n",
    "            if ch.HasTitle and getattr(ch.ChartTitle, \"Text\", \"\") == \"Efficient Frontier\":\n",
    ...
    "\n",
    "        # Range for the frontier points (σ, μ) you’ve written on OPT\n",
    "        x_rng = opt_sheet.range(f\"C{start_s_row}:C{start_s_row + len(stats_df) - 1}\").api  # Vol column\n",
    "        y_rng = opt_sheet.range(f\"B{start_s_row}:B{start_s_row + len(stats_df) - 1}\").api  # Return column\n",
    "\n",
    "        ch = the_chart.Chart\n",
    "        ch.SeriesCollection().Delete()\n",
    "        s1 = ch.SeriesCollection().NewSeries()\n",
    "        s1.XValues = x_rng; s1.Values = y_rng; s1.Name = \"Frontier\"\n",
    "\n",
    "        # Tangency point (if available)\n",
    "        if tan_ret is not None and tan_vol is not None:\n",
    "            s2 = ch.SeriesCollection().NewSeries()\n",
    "            s2.XValues = [tan_vol]; s2.Values = [tan_ret]; s2.Name = \"Tangency\"\n",
    "\n",
    "        # Current portfolio point (if available)\n",
    "        if current_point:\n",
    "            s3 = ch.SeriesCollection().NewSeries()\n",
    "            s3.XValues = [current_point[0]]; s3.Values = [current_point[1]]; s3.Name = \"Current\"\n",
    "\n",
    "        # Title with rf label\n",
    ...
    "        seed_units, seed_include = _read_holdings_seed_from_sheet(wb, \"Holdings\")\n",
    "        wb.close()\n",
    "except Exception:\n",
    ...
    "            # enforce consistent index name for downstream joins/writes\n",
    ...
    "            row_b = np.where(trade_rec[\"Δ Units\"].astype(int) == 0, 0.0, row_b)\n",
