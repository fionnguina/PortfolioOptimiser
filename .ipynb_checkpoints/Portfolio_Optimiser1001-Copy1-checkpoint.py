 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "# Grabbing Data From Yahoo Finance For Stock Build last updated 10/01/2026"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "    if not bool(globals().get(\"OPEN_PPT_AFTER_SAVE\", True)):\n",
    "        return\n",
    "\n",
    "    # Prefer PowerPoint via COM on Windows if available\n",
    "    if globals().get(\"HAS_WIN32COM\", False):\n",
    "        try:\n",
    "            app = win32.Dispatch(\"PowerPoint.Application\")\n",
    "            app.Visible = True\n",
    "            app.Presentations.Open(pptx_path, WithWindow=True)\n",
    "            return\n",
    "        except Exception as e:\n",
    "            print(f\"[ppt] COM open failed, fallback to OS open: {e}\")\n",
    "\n",
    "    _os_open(pptx_path)\n",
    "\n",
    ...
    "# -------------------------------\n",
    "TRADE_PLAN_MODE = \"auto\"   # \"auto\", \"no_tilts\", \"with_tilts\"\n",
    "VALIDATION_LOOKBACK_DAYS = 252  # 1y of daily data (change if you like)\n",
    "\n",
    "# -------------------------------\n",
    "# Trade plan builder\n",
    "# -------------------------------\n",
    ...
    "\n",
    "    \"\"\"\n",
    "    Build a simple rebalance trade list from current units -> target weights.\n",
    "\n",
    "    Returns a DataFrame with columns:\n",
    "      Security, Price, CurrUnits, CurrValue, CurrW, TgtW, TgtValue, DeltaValue, TradeUnits, Side\n",
    "    \"\"\"\n",
    "    # Align index set\n",
    "    current_units = pd.Series(current_units, dtype=float)\n",
    "    last_prices   = pd.Series(last_prices, dtype=float)\n",
    ...
    "    u = current_units.reindex(universe).fillna(0.0)\n",
    "    p = last_prices.reindex(universe).astype(float)\n",
    "\n",
    "    # Drop anything with no price (can’t trade it)\n",
    "    tradable = p.replace([np.inf, -np.inf], np.nan).notna() & (p > 0)\n",
    "    u = u.loc[tradable]\n",
    "    p = p.loc[tradable]\n",
    "\n",
    "    w_tgt = target_weights.reindex(u.index).fillna(0.0)\n",
    "\n",
    "    # Normalise target weights to (1 - cash_buffer)\n",
    "    w_sum = float(w_tgt.sum())\n",
    "    if w_sum > 0:\n",
    "        w_tgt = w_tgt / w_sum\n",
    "    w_tgt = w_tgt * float(1.0 - cash_buffer)\n",
    "\n",
    "    curr_value = u * p\n",
    "    port_value = float(curr_value.sum())\n",
    "    try:\n",
    ...
    "        cur_val = float(pvo)\n",
    "        port_value = float(portfolio_value_override)\n",
    "        \n",
    "    if port_value <= 0:\n",
    "        out = pd.DataFrame({\n",
    "            \"Security\": u.index,\n",
    ...
    "        # round towards zero to avoid overspend/oversell due to rounding\n",
    "        trade_units = np.where(trade_units >= 0, np.floor(trade_units), np.ceil(trade_units))\n",
    "\n",
    "    # Apply min trade filter (in AUD notional)\n",
    "    notional = np.abs(trade_units * p)\n",
    "    trade_units = np.where(notional >= float(min_trade_aud), trade_units, 0.0)\n",
    "\n",
    ...
    "    })\n",
    "\n",
    "    # Helpful ordering: sells first (fund buys), then buys\n",
    ...
    "    return out\n",
    "\n",
    "def _ann_sharpe(r: pd.Series, rf_annual: float) -> float:\n",
    ...
    "        return float(\"nan\")\n",
    "    return float(ex.mean() / vol) * np.sqrt(252.0)\n",
    "\n",
    ...
    "    \"\"\"\n",
    "    Simple validation: compare realised Sharpe over the last lookback_days.\n",
    "    Returns: (choice_label, chosen_weights, diagnostics_dict)\n",
    "    \"\"\"\n",
    "    r = returns_df.tail(int(lookback_days)).copy()\n",
    ...
    "    p1 = (r @ w1).dropna()\n",
    "\n",
    "    sh0 = _ann_sharpe(p0, rf_annual)\n",
    "    sh1 = _ann_sharpe(p1, rf_annual)\n",
    "\n",
    "    diag = {\"sharpe_no_tilts\": sh0, \"sharpe_with_tilts\": sh1}\n",
    "\n",
    "    # choose the best Sharpe (fallback to no-tilts if NaNs)\n",
    "    if np.isfinite(sh1) and (not np.isfinite(sh0) or sh1 > sh0):\n",
    "        return \"with_tilts\", w1, diag\n",
    "    return \"no_tilts\", w0, diag\n",
    "\n",
    "    # --- Build all trade plans ---\n",
    "    tp_no_tilts  = build_trade_plan_from_units(units_series, last_px, w_no_tilts, cash_buffer=0.00, min_trade_aud=200, portfolio_value_override=portfolio_value_override)\n",
    "    tp_with_tilts = build_trade_plan_from_units(units_series, last_px, w_with_tilts, cash_buffer=0.00, min_trade_aud=200, portfolio_value_override=portfolio_value_override)    \n",
    "    choice_label, w_chosen, diag = choose_portfolio_for_tradeplan(\n",
    "        returns_df=returns_wide_df,  # your wide returns matrix\n",
    "        w_no_tilts=w_no_tilts,\n",
    "        w_with_tilts=w_with_tilts,\n",
    "        rf_annual=rf_annual,\n",
    "        lookback_days=VALIDATION_LOOKBACK_DAYS\n",
    "    )\n",
    "    tp_chosen = build_trade_plan_from_units(units_series, last_px, w_chosen, cash_buffer=0.00, min_trade_aud=200)\n",
    "\n",
    "\n",
    "# ---------------------------------------------------------------------\n",
    "# Actual Code\n",
   ...
   "outputs": [
    {
     ...
     "text": [
      ...
      "Tickers loaded from sheet: ['A200.AX', 'IEU.AX', 'IJP.AX', 'IOO.AX', 'IVV.AX', 'MTUM.AX', 'MVW.AX', 'QHAL.AX', 'QLTY.AX', 'QUAL.AX', 'SMH', 'SMLL.AX', 'SPY', 'VAS.AX', 'VDHG.AX', 'VGE.AX', 'VGS.AX', 'VLUE.AX', 'VMIN.AX', 'VSO.AX', 'VVLU.AX', '^AORD']\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        \"\"\"\n",
    "        tickers_all = [t for t in prices.columns if (t != \"PortfolioValue\") and (not str(t).startswith(\"^\"))]\n",
    ...
    "            if disabled:\n",
    "                v_units = _tk.StringVar(value=\"0\")\n",
    "            else:\n",
    "                try:\n",
    "                    u0 = float(units_default)\n",
    "                    # show as integer when it is effectively an integer\n",
    "                    v_units = _tk.StringVar(value=str(int(round(u0))) if abs(u0 - round(u0)) < 1e-9 else str(u0))\n",
    "                except Exception:\n",
    "                    v_units = _tk.StringVar(value=\"0\")\n",
    ...
    "                    v_inc.set(False)  # force include off when deleted\n",
    "                else:\n",
    "                    row_frame.grid()  # show again if unticked\n",
    "            \n",
    ...
    "            ent     = _ttk.Entry(row_frame, textvariable=v_units, width=16)\n",
    ...
    "            ent.grid(row=0, column=3, sticky=\"w\", padx=(0, 6))\n",
    "            lbl_px.grid(row=0, column=4, sticky=\"w\", padx=(0, 6))\n",
    "            row_vars[ticker] = {\"inc\": v_inc, \"del\": v_del, \"units\": v_units, \"disabled\": disabled, \"lbl_px\": lbl_px, \"row_frame\": row_frame, \"ent_units\": ent}\n",
    "    \n",
    "        # Prefill rows\n",
    ...
    "            t = t.upper()\n",
    "            if t in row_vars:\n",
    "                # If it exists but was deleted/hidden, restore it\n",
    "                vs = row_vars[t]\n",
    "                if bool(vs[\"del\"].get()):\n",
    "                    vs[\"del\"].set(False)\n",
    "                    vs[\"inc\"].set(True)\n",
    "                    if \"row_frame\" in vs:\n",
    "                        vs[\"row_frame\"].grid()  # show again\n",
    ...
    "    \n",
    "        # -------------------------------\n",
    "        # Options (placed above the bottom buttons)\n",
    "        # -------------------------------\n",
    "        frm_opts = _ttk.Frame(root, padding=(10, 0, 10, 0))\n",
    "        frm_opts.pack(fill=\"x\")\n",
    "        \n",
    "        _ttk.Checkbutton(frm_opts, text=\"Open Excel after Save\", variable=open_excel_var).pack(anchor=\"w\")\n",
    "        _ttk.Checkbutton(frm_opts, text=\"Open PowerPoint after Save\", variable=open_ppt_var).pack(anchor=\"w\")\n",
    "    \n",
    ...
    "            \n",
    "            # Portfolio value override (optional)\n",
    "            portfolio_value_override = None\n",
    "            if bool(use_portfolio_value.get()):\n",
    "                try:\n",
    "                    raw = str(portfolio_value_var.get())\n",
    "                    raw = raw.replace(\",\", \"\").replace(\"$\", \"\").replace(\"AUD\", \"\").strip()\n",
    "                    portfolio_value_override = float(raw)                \n",
    "                except ValueError:\n",
    "                    portfolio_value_override = None\n",
    "            \n",
    "            edit_holdings_and_tilts_dialog.result = (\n",
    "                units_ser, last_price_ser, prices, include_flags, tilts_df, portfolio_value_override\n",
    "            )\n",
    "            root.destroy()\n",
    "    \n",
  ...
  },
  {
   ...
   "outputs": [
    {
     ...
     "output_type": "stream",
     "text": [
      "Using FX-adjusted returns for Sigma?: True (max |diff|=2.98e-19)\n",
      "[frontier] target_returns from 3.6000% to 35.7797%\n",
      "[frontier] tangency ret ≈ 0.2223, vol ≈ 0.0372\n",
      "\n",
      "--- DEBUG CHECK: Sigma_opt / mu_vec_opt ---\n",
      "Any NaN in Sigma_opt: False\n",
      "Any NaN in mu_vec_opt: False\n",
      "Min variance: 4.0141289033712567e-05\n",
      "Number of assets: 21\n",
      "Security       A200.AX        IEU.AX    IJP.AX        IOO.AX        IVV.AX  \\\n",
      "Security                                                                     \n",
      "A200.AX   5.751241e-05  2.003183e-07  0.000003  1.248088e-06  1.370060e-06   \n",
      "IEU.AX    2.003183e-07  4.723797e-05  0.000002  4.234352e-07  3.076065e-07   \n",
      "IJP.AX    3.483581e-06  2.024467e-06  0.000147  4.845308e-06  4.882687e-06   \n",
      "IOO.AX    1.248088e-06  4.234352e-07  0.000005  8.864994e-05  2.094993e-06   \n",
      "IVV.AX    1.370060e-06  3.076065e-07  0.000005  2.094993e-06  8.281683e-05   \n",
      "\n",
      "Security       MTUM.AX        MVW.AX       QHAL.AX       QLTY.AX  \\\n",
      "Security                                                           \n",
      "A200.AX   1.867942e-06  6.555632e-07  1.112712e-06  1.356832e-06   \n",
      "IEU.AX    7.035840e-07 -2.056479e-07  2.085330e-07  5.572266e-07   \n",
      "IJP.AX    7.442633e-06 -3.706536e-08  2.428151e-06  6.017906e-06   \n",
      "IOO.AX    2.320800e-06  8.251565e-07  1.703837e-06  1.958480e-06   \n",
      "IVV.AX    2.162379e-06  1.555368e-06  1.822048e-06  2.190233e-06   \n",
      "\n",
      "Security       QUAL.AX  ...       SMLL.AX       SPY        VAS.AX  \\\n",
      "Security                ...                                         \n",
      "A200.AX   2.043640e-06  ...  2.628254e-07  0.000007  1.199878e-06   \n",
      "IEU.AX    9.325102e-07  ... -3.198904e-07  0.000004  2.327356e-07   \n",
      "IJP.AX    9.616272e-06  ... -3.113612e-06  0.000036  3.632888e-06   \n",
      "IOO.AX    3.186991e-06  ... -6.001752e-07  0.000005  1.252865e-06   \n",
      "IVV.AX    3.552234e-06  ... -7.369832e-07  0.000002  1.374704e-06   \n",
      "\n",
      "Security       VDHG.AX        VGE.AX        VGS.AX   VLUE.AX       VMIN.AX  \\\n",
      "Security                                                                     \n",
      "A200.AX   9.834436e-07 -2.575365e-08  1.251843e-06  0.000002  4.696682e-07   \n",
      "IEU.AX    2.106278e-07  3.023155e-07  4.385042e-07  0.000001 -1.115112e-07   \n",
      "IJP.AX    2.981363e-06  1.254092e-06  5.226535e-06  0.000010 -3.738796e-08   \n",
      "IOO.AX    1.274301e-06 -4.477942e-07  1.913703e-06  0.000003  5.212732e-07   \n",
      "IVV.AX    1.448298e-06 -8.969807e-07  2.208174e-06  0.000003  7.906773e-07   \n",
      "\n",
      "Security        VSO.AX   VVLU.AX  \n",
      "Security                          \n",
      "A200.AX   5.748234e-07  0.000002  \n",
      "IEU.AX   -8.793376e-08  0.000001  \n",
      "IJP.AX   -4.912055e-08  0.000011  \n",
      "IOO.AX    2.946935e-07  0.000003  \n",
      "IVV.AX    4.354832e-07  0.000004  \n",
      "\n",
      "[5 rows x 21 columns]\n",
      "Security\n",
      "A200.AX    0.140630\n",
      ...
      "IVV.AX     0.272836\n",
      "dtype: float64\n",
      "OPT TICKERS: ['A200.AX', 'IEU.AX', 'IJP.AX', 'IOO.AX', 'IVV.AX', 'MTUM.AX', 'MVW.AX', 'QHAL.AX', 'QLTY.AX', 'QUAL.AX', 'SMH', 'SMLL.AX', 'SPY', 'VAS.AX', 'VDHG.AX', 'VGE.AX', 'VGS.AX', 'VLUE.AX', 'VMIN.AX', 'VSO.AX', 'VVLU.AX']\n",
      "mu: count    21.000000\n",
      "mean      0.225235\n",
      "std       0.088584\n",
      "min       0.123825\n",
      ...
      "max       0.529653\n",
      "dtype: float64\n",
      "Sigma diag min/max: 4.0141289033712567e-05 0.0006892070463974889\n",
      "Mkt-RF    0.188673\n",
      "SMB      -0.050877\n",
      "HML       0.013623\n",
      "RMW      -0.053427\n",
   ...
   "source": [
    ...
    "gamma_cgt = 1.0\n",
    "rf_label = f\"{rf_annual*100:.2f}%\"\n",
    "chart_title = f\"Efficient Frontier\"\n",
    "\n",
    ...
    "\n",
    "    # Guardrail: PortfolioValue must never be an asset in optimisation inputs\n",
    ...
    "    tilt_mode: str = \"soft\",          # \"soft\" (recommended) or \"hard\"\n",
    "    tilt_penalty: float = 1e4         # higher = stronger push to meet bands\n",
    ") -> tuple[np.ndarray, bool, str]:\n",
    "    \"\"\"\n",
    "    Markowitz long-only solver using CVXPY with optional factor-tilt constraints.\n",
    "\n",
    "    minimise:     w' Σ w   +  tilt_penalty * (sum of tilt band violations)   [soft mode]\n",
    "    subject to:   sum(w) = 1\n",
    "                  w >= 0\n",
    "                  μ·w >= target_return   (or == if use_inequality=False)\n",
    "                  (optional) factor tilts within [target ± band] (hard or soft)\n",
    "    \"\"\"\n",
    "    import cvxpy as cp\n",
    "\n",
    "    mu = pd.Series(mu).reindex(Sigma.index).astype(float)\n",
    ...
    "    if use_inequality:\n",
    "        constraints.append(mu.values @ w >= float(target_return))\n",
    "    else:\n",
    "        constraints.append(mu.values @ w == float(target_return))\n",
    "\n",
    "    # Optional tilt constraints\n",
    "    slack_terms = []\n",
    "    if B is not None and tilt_targets is not None and tilt_bands is not None:\n",
    "        B_use = B.reindex(mu.index).copy()\n",
    "        if use_mask is None:\n",
    "            use_mask = {}\n",
    "\n",
    "        # normalise inputs\n",
    ...
    "                continue\n",
    "\n",
    "            t = float(tilt_targets.loc[f]) if pd.notna(tilt_targets.loc[f]) else 0.0\n",
    "            b = float(tilt_bands.loc[f])   if pd.notna(tilt_bands.loc[f])   else 0.0\n",
    "            v = pd.to_numeric(B_use[f], errors=\"coerce\").fillna(0.0).values\n",
    "\n",
    "            if tilt_mode.lower() == \"hard\":\n",
    "                constraints.append(v @ w <= t + b)\n",
    "                constraints.append(v @ w >= t - b)\n",
    "            else:\n",
    "                # soft constraints: allow violations but penalise them\n",
    ...
    "                slack_terms.append(s_pos + s_neg)\n",
    "\n",
    "    # Objective (variance + slack penalty in soft mode)\n",
    "    obj = cp.quad_form(w, Sigma.values)\n",
    "    if slack_terms and tilt_mode.lower() == \"soft\":\n",
    "        obj = obj + float(tilt_penalty) * cp.sum(cp.hstack(slack_terms))\n",
    "\n",
    ...
    "\n",
    "    return np.asarray(w.value).flatten(), True, note\n",
    "\n",
    "\n",
    ...
    "        for f in tilt_targets.index:\n",
    "            if not use_mask.get(f, False):\n",
    "                continue\n",
    "            if f not in B.columns:\n",
    "                continue\n",
    "\n",
    "            v = B[f].reindex(mu.index).fillna(0.0).values\n",
    "            t = float(tilt_targets.loc[f])\n",
    "            b = float(tilt_bands.loc[f])\n",
    "\n",
    "            constraints.append(v @ w <= t + b)\n",
    "            constraints.append(v @ w >= t - b)\n",
    "\n",
    ...
    "        return np.full(n, np.nan), False, \"Infeasible\"\n",
    "\n",
    "    return np.asarray(w.value).flatten(), True, \"CVXPY (tilts)\"\n",
    "\n",
    ...
    "    \n",
    "        # skip factors not available in B\n",
    ...
    "    \n",
    "    # Build tilt inputs ONCE (safe defaults if tilt_df missing)\n",
    ...
    "        w, ok, note = solve_frontier_point_cvxpy(\n",
    "            mu_vec_opt,\n",
    "            Sigma_opt,\n",
    "            R\n",
    "        )\n",
    ...
    "            \"Sharpe\":            sharpe,\n",
    "            \"Method\":            \"Frontier CVXPY (soft tilts)\" if (tilt_targets is not None and use_mask) else \"Frontier CVXPY\",\n",
    "            \"Note\":              note,\n",
    "        })\n",
    "\n",
    "\n",
    ...
    "\n",
    "def make_trade_plan(units_cur, last_px, fx_map, w_target, include_flags, include_zero_lines=False, portfolio_value_override=None):\n",
    "    \"\"\"\n",
    ...
    "\n",
    "    print(\"[debug] make_trade_plan inputs:\")\n",
    "    print(\"  len(units) =\", len(pd.Series(units)))\n",
    "    print(\"  len(last_px_hold) =\", len(pd.Series(last_px_hold)))\n",
    "    print(\"  len(include_flags) =\", len(pd.Series(include_flags)))\n",
    "    \n",
    "    print(\"  w_target type =\", type(w_star_with_tilts))\n",
    "    print(\"  w_target len =\", len(w_star_with_tilts))\n",
    "    print(\"  w_target sum =\", float(pd.Series(w_star_with_tilts).sum()))\n",
    "    print(\"  w_target nonzero =\", int((pd.Series(w_star_with_tilts).abs() > 1e-12).sum()))\n",
    "    print(\"  w_target NaN =\", int(pd.Series(w_star_with_tilts).isna().sum()))\n",
    "    print(\"  w_target head =\", pd.Series(w_star_with_tilts).head(5).to_dict())\n",
    "\n",
    "    tgt_idx = set(pd.Series(w_star_with_tilts).index.astype(str))\n",
    "    unit_idx = set(pd.Series(units).index.astype(str))\n",
    "    px_idx = set(pd.Series(last_px_hold).index.astype(str))\n",
    "    \n",
    "    print(\"  overlap w_target vs units =\", len(tgt_idx & unit_idx))\n",
    "    print(\"  overlap w_target vs prices =\", len(tgt_idx & px_idx))\n",
    "    print(\"  sample missing in units =\", list(sorted(tgt_idx - unit_idx))[:5])\n",
    "    print(\"  sample missing in prices =\", list(sorted(tgt_idx - px_idx))[:5])\n",
    "\n",
    "    \n",
    "def compute_target_units_for_holdings(units_cur, last_px, fx_map, w_target, include_flags, portfolio_value_override=None):\n",
    "    tickers = list(pd.Index(w_target.index))\n",
    "    print(\"\\n--- DEBUG: make_trade_plan() ---\")\n",
    "    print(\"len(w_target) =\", len(w_target), \"sum(w_target) =\", float(pd.Series(w_target).sum()))\n",
    "    print(\"nonzero(w_target) =\", int((pd.Series(w_target).abs() > 1e-12).sum()))\n",
    "    print(\"w_target head:\", pd.Series(w_target).head(5).to_dict())\n",
    ...
    "    print(\"len(lp_aud) =\", len(lp_aud), \"nan(lp_aud) =\", int(pd.Series(lp_aud).isna().sum()))\n",
    "    print(\"sum(cur_units) =\", float(cur_units.sum()), \"cur_val (pre-override) =\", float((cur_units * lp_aud).sum()))\n",
    "    print(\"portfolio_value_override =\", portfolio_value_override)\n",
    "\n",
    "    cur_val = float((cur_units * lp_aud).sum())\n",
    "    if portfolio_value_override is not None and np.isfinite(portfolio_value_override) and portfolio_value_override > 0:\n",
    "        cur_val = float(portfolio_value_override)  \n",
    "        print(\"cur_val (post-override) =\", cur_val)\n",
    "\n",
    "    if cur_val <= 0:\n",
    ...
    "        w_aligned = pd.Series(w_target).reindex(tickers)\n",
    "        print(\"aligned sum(w_target) =\", float(w_aligned.fillna(0.0).sum()),\n",
    "              \"nan(aligned) =\", int(w_aligned.isna().sum()),\n",
    "              \"nonzero(aligned) =\", int((w_aligned.fillna(0.0).abs() > 1e-12).sum()))\n",
    "        \n",
    ...
    "    fac = f_mean_ann.index\n",
    "    mu = f_mean_ann.values\n",
    "    Sigma = (Fcov_daily.loc[fac, fac].values) * 252.0\n",
    "    Sigma_inv = np.linalg.pinv(Sigma)\n",
    "    t_opt = Sigma_inv @ mu\n",
    "\n",
    ...
    "    try:\n",
    "        mins = pd.Series(B[fac].min(axis=0), index=fac)\n",
    "        maxs = pd.Series(B[fac].max(axis=0), index=fac)\n",
  ...
  },
  {
   ...
   "outputs": [
    {
     ...
     "text": [
      ...
      "[debug] len(Sigma_opt.index) = 21 | type(w_tilt) = <class 'numpy.ndarray'> | len(w_tilt) = 21\n",
      "[debug] tilt_point=(0.07260187643110265, 0.2476187562348507), factor_point=None\n",
      "[pptx] Tilt table storage skipped: 'Without Tilts'\n",
      "1 name: Chart 2 | title: Efficient Frontier\n",
      "\n",
      "=== LOT-EXPANDED TABLE ===\n",
      "   Security    AcqDate  UnitsSold    AcqPrice     CostBase  Last Px (AUD)  \\\n",
      "0   A200.AX        NaT         21         NaN          NaN     144.970001   \n",
      "1    IEU.AX 2025-12-15         58  101.480003  5885.840195     103.040001   \n",
      "2    IEU.AX 2025-12-22         38  102.989998  3913.619919     103.040001   \n",
      "3    IJP.AX        NaT          6         NaN          NaN     123.089996   \n",
      "4    IOO.AX 2025-12-15         17  187.369995  3185.289917     188.179993   \n",
      "5    IOO.AX 2025-12-22          6  188.419998  1130.519989     188.179993   \n",
      "6    IOO.AX 2025-12-26         17  187.619995  3189.539917     188.179993   \n",
      "7    IOO.AX 2025-12-26          2  187.619995   375.239990     188.179993   \n",
      "8    IVV.AX 2025-12-15         77   68.699997  5289.899765      68.779999   \n",
      "9    IVV.AX 2025-12-22         25   69.050003  1726.250076      68.779999   \n",
      "10  MTUM.AX        NaT         25         NaN          NaN      28.660000   \n",
      "11   MVW.AX 2025-12-15         21   38.619999   811.019978      38.299999   \n",
      "12   MVW.AX 2025-12-22         19   38.529999   732.069977      38.299999   \n",
      "13   MVW.AX 2025-12-22         17   38.599998   656.199974      38.299999   \n",
      "14  QHAL.AX 2025-12-15         28   52.939999  1482.319962      53.980000   \n",
      "15  QHAL.AX 2025-12-22         27   53.049999  1432.349979      53.980000   \n",
      "16  QLTY.AX 2025-11-21         28   32.330002   905.240051      33.549999   \n",
      "17  QLTY.AX 2025-12-15        125   33.224998  4153.124809      33.549999   \n",
      "18  QLTY.AX 2025-12-22         11   33.360001   366.960007      33.549999   \n",
      "19  QUAL.AX 2025-12-15         48   61.880001  2970.240051      62.560001   \n",
      "\n",
      "    Cash Flow (AUD)  Brokerage (AUD)  RealisedGain  \n",
      "0      -3044.370026             11.0           NaN  \n",
      "1      -9891.840088             11.0     90.479858  \n",
      "2      -9891.840088             11.0      1.900116  \n",
      "3       -738.539978             11.0           NaN  \n",
      "4      -7903.559692             11.0     13.769958  \n",
      "5      -7903.559692             11.0     -1.440033  \n",
      "6      -7903.559692             11.0      9.519958  \n",
      "7      -7903.559692             11.0      1.119995  \n",
      "8      -7015.559875             11.0      6.160141  \n",
      "9      -7015.559875             11.0     -6.750107  \n",
      "10      -716.499996             11.0           NaN  \n",
      "11     -2183.099957             11.0     -6.719994  \n",
      "12     -2183.099957             11.0     -4.369991  \n",
      "13     -2183.099957             11.0     -5.099987  \n",
      "14     -2968.899975             11.0     29.120026  \n",
      "15     -2968.899975             11.0     25.110008  \n",
      "16     -5502.199875             11.0     34.159927  \n",
      "17     -5502.199875             11.0     40.625095  \n",
      "18     -5502.199875             11.0      2.089985  \n",
      "19     -5129.920113             11.0     32.640015  \n",
      "[debug] Current totals → Portfolio: -102328.41, Net Invested: 0.00\n",
      "[debug] Previous totals → Portfolio: -102328.41, Net Invested: 0.00\n",
      "[debug] Saved new state → Portfolio: -102328.41, Net Invested: 0.00\n",
      "[pptx prep] PortfolioValue series computed for 21 securities.\n",
      "[ppt] Report saved to: C:\\Users\\Fionn Guina\\Portfolio_Optimiser\\Reports\\Portfolio_Report.pptx\n",
      "Workbook Successfully Updated\n",
      "[shortcut] skipped due to error: unsupported operand type(s) for /: 'str' and 'str'\n",
      "=== MU VEC (sorted) ===\n",
      "Security\n",
      "MTUM.AX    0.123825\n",
      "MVW.AX     0.125768\n",
      ...
      "dtype: float64\n",
      "\n",
      "Min mu: 0.12382465686292508\n",
      "Max mu: 0.5296527984092879\n",
      "Mean mu: 0.22523513073175483\n",
   ...
   "source": [
    ...
    "            \n",
    "            w_star_with_tilts = None\n",
    "            \n",
    "            # Use tilt solution if it exists and is usable; otherwise fall back to no-tilts weights\n",
    "            if (\"w_tilt\" in locals()) and (w_tilt is not None):\n",
    "                # Robust alignment: handle Series vs array, and common \"extra element\" cases\n",
    "                if isinstance(w_tilt, pd.Series):\n",
    "                    w_star_with_tilts = pd.to_numeric(w_tilt, errors=\"coerce\").reindex(Sigma_opt.index).fillna(0.0)\n",
    "                else:\n",
    "                    wt = np.asarray(w_tilt, dtype=float).reshape(-1)\n",
    "                    if wt.shape[0] == len(Sigma_opt.index):\n",
    "                        w_star_with_tilts = pd.Series(wt, index=Sigma_opt.index).fillna(0.0)\n",
    "                    elif wt.shape[0] == len(Sigma_opt.index) + 1:\n",
    "                        # Common case: solver returned an extra cash/RF element at the end\n",
    "                        w_star_with_tilts = pd.Series(wt[:len(Sigma_opt.index)], index=Sigma_opt.index).fillna(0.0)\n",
    "                    else:\n",
    "                        raise ValueError(f\"w_tilt length {wt.shape[0]} does not match Sigma_opt universe {len(Sigma_opt.index)}\")\n",
    "            \n",
    "                # Normalise if non-zero\n",
    "                s = float(w_star_with_tilts.sum())\n",
    "                if s != 0.0:\n",
    "                    w_star_with_tilts = w_star_with_tilts / s\n",
    "            else:\n",
    "                # fallback: if tilt solve failed or missing, default to no-tilts\n",
    "                w_star_with_tilts = w_star_no_tilts.copy()\n",
    "            \n",
    ...
    "            w_star_no_tilts, _, _ = solve_frontier_point_cvxpy_with_tilts(\n",
    "                mu_vec_opt,\n",
    "                Sigma_opt,\n",
    "                R_star,\n",
    "                B,\n",
    ...
    "            w_star_with_tilts, _, _ = solve_frontier_point_cvxpy_with_tilts(\n",
    "                mu_vec_opt,\n",
    "                Sigma_opt,\n",
    "                R_star,\n",
    "                B,\n",
    ...
    "            \n",
    "                # ---------- With Tilts achieved tilts ----------\n",
    "                if (\"w_tilt\" in locals()) and (w_tilt is not None) and np.all(np.isfinite(w_tilt)):\n",
    "                    w_with = _norm_w(w_tilt, Sigma_opt.index).reindex(B.index).fillna(0.0)\n",
    "                    if float(w_with.sum()) != 0:\n",
    "                        w_with = w_with / float(w_with.sum())\n",
    "            \n",
    "                    with_beta = (B.T @ w_with).rename(\"With Tilts\")\n",
    "                    with_beta = with_beta.reindex([f for f in factor_order if f in with_beta.index])\n",
    "            \n",
    "                    tilt_tbl = pd.DataFrame({\"Achieved β\": with_beta})\n",
    "            \n",
    "                    # If tilt targets exist, include them (and Diff)\n",
    "                    if isinstance(tilt_df, pd.DataFrame) and (not tilt_df.empty) and (\"Target\" in tilt_df.columns):\n",
    "                        tgt = tilt_df.reindex(tilt_tbl.index)\n",
    "                        tilt_tbl[\"Target β\"] = pd.to_numeric(tgt[\"Target\"], errors=\"coerce\")\n",
    "                        tilt_tbl[\"Diff\"] = tilt_tbl[\"Achieved β\"] - tilt_tbl[\"Target β\"]\n",
    "            \n",
    "                        # Optional: store only factors marked Use? == True\n",
    "                        if \"Use?\" in tgt.columns:\n",
    "                            use_mask = tgt[\"Use?\"].astype(bool)\n",
    "                            tilt_tbl = tilt_tbl.loc[use_mask.reindex(tilt_tbl.index).fillna(False)]\n",
    "            \n",
    "                    charts[\"with_tilts_achieved_tilts\"] = (\n",
    "                        tilt_tbl.reset_index()\n",
    "                        .rename(columns={\"index\": \"Factor\"})\n",
    "                        .to_dict(\"records\")\n",
    "                    )\n",
    "            \n",
    "                # ---------- Achieved tilts table (WITH TILTS ONLY) ----------\n",
    "                out = pd.DataFrame(index=[f for f in factor_order if f in B.columns])\n",
    "                \n",
    "                # Achieved β from the tilted portfolio (w_tilt)\n",
    "                if (\"w_tilt\" in locals()) and (w_tilt is not None) and np.all(np.isfinite(w_tilt)):\n",
    "                    w_with = _norm_w(w_tilt, Sigma_opt.index)\n",
    "                    w_with = w_with.reindex(B.index).fillna(0.0)\n",
    "                    if float(w_with.sum()) != 0:\n",
    "                        w_with = w_with / float(w_with.sum())\n",
    "                \n",
    "                    with_beta = (B.T @ w_with).rename(\"Tilted Portfolio\")\n",
    "                    with_beta = with_beta.reindex(out.index)\n",
    "                    out[\"Tilted Portfolio\"] = with_beta\n",
    "                \n",
    "                # Targets (from Tilts sheet)\n",
    "                if isinstance(tilt_df, pd.DataFrame) and (not tilt_df.empty) and (\"Target\" in tilt_df.columns):\n",
    "                    tgt = tilt_df.reindex(out.index)\n",
    "                    out[\"Target Tilt\"] = pd.to_numeric(tgt[\"Target\"], errors=\"coerce\")\n",
    "                \n",
    ...
    "                        out = out.loc[use_mask.reindex(out.index).fillna(False)]\n",
    "            \n",
    "                    if \"With Tilts\" in out.columns:\n",
    "                        out[\"With - Target\"] = out[\"With Tilts\"] - out[\"Target\"]\n",
    "                    out[\"Without - Target\"] = out[\"Without Tilts\"] - out[\"Target\"]\n",
    "            \n",
    ...
    "                \n",
    "                    # Ensure weights align to B.index (factor exposure universe)\n",
    "                    w_use = w_use.reindex(B.index).fillna(0.0)\n",
    "                \n",
    "                    s = float(w_use.sum())\n",
    "                    if s > 0:\n",
    "                        w_use = w_use / s\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "OPEN_EXCEL_AFTER_SAVE = bool(CFG.get(\"open_excel_after_save\", True))\n",
    "OPEN_PPT_AFTER_SAVE   = bool(CFG.get(\"open_ppt_after_save\", True))\n",
    "\n",
    "def _os_open(path):\n",
    "    try:\n",
    "        # Windows\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    p.alignment = 1  # centre\n",
    "\n",
    "def _set_left_border(cell, width_emu=38100):  # ~3pt\n",
    "    tc = cell._tc\n",
    "    tcPr = tc.get_or_add_tcPr()\n",
    "    lnL = OxmlElement(\"a:lnL\")\n",
    "    lnL.set(\"w\", str(int(width_emu)))\n",
    "\n",
    "    solidFill = OxmlElement(\"a:solidFill\")\n",
    "    srgbClr = OxmlElement(\"a:srgbClr\")\n",
    "    srgbClr.set(\"val\", \"FFFFFF\")  # white line (works well on your blue header)\n",
    ...
    "        if title:\n",
    "            tb = slide.shapes.add_textbox(left, top - Inches(0.3), width, Inches(0.3))\n",
    "            tf = tb.text_frame\n",
    "            tf.clear()\n",
    "            p = tf.paragraphs[0]\n",
    "            p.text = title\n",
    ...
    "            top = Cm(4.0)\n",
    "            left = Cm(0.25 + idx * 4.75)  # tighter to edge\n",
    "            width = Cm(7.58)\n",
    "            height = Cm(11.94)\n",
    "        \n",
    ...
    "        pval_all = pd.to_numeric(portfolio_value_series, errors=\"coerce\").dropna().copy()\n",
    ...
    "        try:\n",
    "            # We need returns matrix and weights; use globals if you have them\n",
    "            returns_wide_df = globals().get(\"returns_wide_df\", None)\n",
    "            w_tilt = globals().get(\"w_tilt\", None)\n",
    "        \n",
    "            if isinstance(returns_wide_df, pd.DataFrame) and w_tilt is not None:\n",
    "                w_ser = pd.Series(w_tilt, index=returns_wide_df.columns).astype(float).fillna(0.0)\n",
    "                if float(w_ser.sum()) != 0:\n",
    "                    w_ser = w_ser / float(w_ser.sum())\n",
    "        \n",
    "                r_tilt = (returns_wide_df.reindex(pval.index).fillna(0.0) @ w_ser).astype(float)\n",
    "                tilted_curve = (1.0 + r_tilt).cumprod()\n",
    "                tilted_curve = tilted_curve / float(tilted_curve.iloc[0])\n",
    "                tilted_returns = tilted_curve - 1.0\n",
    "        \n",
    "                # Add it into the dataframe later as \"With Tilts\"\n",
    "                # (we’ll stitch it in just before perf_df is built)\n",
    "            else:\n",
    "                tilted_returns = None\n",
    "        except Exception:\n",
    "            tilted_returns = None\n",
    ...
    "\n",
    "            \n",
    "            # Benchmarks: download a longer window so 6M/12M/3Y aren’t clipped to the same start date\n",
    "            bench_px = {}\n",
    ...
    "            plt.close(fig)\n",
    "            \n",
    "            slide4.shapes.add_picture(ff_chart_path, Inches(0.8), Inches(1.15), width=Inches(8.2), height=Inches(3.65))\n",
    "            \n",
    ...
    "            \n",
    "            rows = []\n",
    "            for k in [\"Current\", \"Previous\", \"With Tilts\", \"Target\"]: #Change these for the names\n",
    "                v = pts.get(k, None)\n",
    "                if v is None:\n",
    "                    continue\n",
    "                try:\n",
    ...
    "\n",
    "                        # --- reshape columns for readability ---\n",
    "                        rename_map = {\n",
    "                            \"With Tilts\": \"Tilted Portfolio\",\n",
    "                            \"Target\": \"Target Tilt\",\n",
    "                            \"Without Tilts\": \"Optimised Portfolio\",\n",
    "                        }\n",
    "                        \n",
    "                        df_tilts = df_tilts.rename(columns=rename_map)\n",
    "                        \n",
    "                        # Keep only the readable columns (drop diffs / other variants)\n",
    "                        keep_cols = [\"Factor\", \"Tilted Portfolio\", \"Target Tilt\", \"Optimised Portfolio\"]\n",
    "                        df_tilts = df_tilts[[c for c in keep_cols if c in df_tilts.columns]]\n",
    "\n",
    "                        \n",
    "                        # Layout: under the points table on the right\n",
    "                        left2 = Cm(16.50)\n",
    "                        top2 = Cm(10.05)\n",
    "                        width2 = Cm(7.72)     # wider to fit multiple columns\n",
    "                        height2 = Cm(5.44)\n",
    "                \n",
    "                        shp2 = slide5.shapes.add_table(df_tilts.shape[0] + 1, df_tilts.shape[1], left2, top2, width2, height2)\n",
    "                        tbl2 = shp2.table\n",
    "\n",
    "                        # --- thick divider between \"Target Tilt\" (col 2) and \"Optimised Portfolio\" (col 3) ---\n",
    "                        divider_x = left2 + tbl2.columns[0].width + tbl2.columns[1].width + tbl2.columns[2].width\n",
    "                        \n",
    "                        divider = slide5.shapes.add_shape(\n",
    "                            MSO_SHAPE.RECTANGLE,\n",
    "                            divider_x,          # x position\n",
    "                            top2,               # same top as table\n",
    "                            Pt(2.5),              # thickness\n",
    "                            height2             # same height as table\n",
    "                        )\n",
    "                        divider.fill.solid()\n",
    "                        divider.fill.fore_color.rgb = RGBColor(0, 51, 102)  # matches your header navy\n",
    ...
    "                                for p in tbl2.cell(rr, cc).text_frame.paragraphs:\n",
    "                                    p.font.size = Pt(9)\n",
    "                                    p.font.bold = True\n",
    "                                    p.alignment = PP_ALIGN.CENTER\n",
    "\n",
    "                        # Thick divider before \"Optimised Portfolio\" column (index 3)\n",
    "                        opt_col = df_tilts.columns.get_loc(\"Optimised Portfolio\")\n",
    "                        for rr in range(df_tilts.shape[0] + 1):\n",
    "                            _set_left_border(tbl2.cell(rr, opt_col), width_emu=57150)  # ~4.5pt\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "if OPEN_PPT_AFTER_SAVE and ppt_path and os.path.exists(ppt_path):\n",
    "    # Wait until the PPTX is fully written (PPTX is a ZIP; opening too early can break rendering)\n",
    "    \n",
    "    def _wait_for_pptx_ready(path, timeout_s=10, stable_s=1.0):\n",
    "        t0 = time.time()\n",
    "        last_size = -1\n",
    ...
    "\n",
    "                # detect size stability\n",
    "                if size != last_size:\n",
    "                    last_size = size\n",
    "                    last_change = time.time()\n",
    "\n",
    "                # only attempt open when size hasn't changed for stable_s\n",
